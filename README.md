# 🚀 DUST Mod Installation Guide

Hi there! 👋  
If you run into any issues or something doesn’t work, don’t worry — just reach out, I’m happy to help!

---

## 1. Install Required Software
- **DUST** — make sure the game is installed.  
- **[MelonLoader](https://github.com/LavaGang/MelonLoader/releases/tag/v0.7.2)** — download version 0.7.2.  
- **[.NET 6](https://dotnet.microsoft.com/en-us/download/dotnet/6.0)** — required for MelonLoader.

## 2. Prepare the Game
- Launch the game at least once, then close it.  
- In MelonLoader, select `Dust.exe`.

## 3. Start patch
- Click **Install** and wait for it to finish.  
- Launch the game — this step will trigger the .NET installation if needed.

## 4. Install the Mod
- [Download](https://github.com/LolegRus/ClearVision-mod-for-DUST/releases/latest/download/Clear_Vision.dll) the mod `.dll` file and place it in the `Mods` folder.  
- Launch the game.

## 5. Verify Mod Functionality
- Open the console.  
- The console should display information about the mod.  
- Errors like **OpenXR** can be ignored — they do not affect mods.  
- If nothing appears, double-check the previous steps.

---

## 💬 Contact Me
If something doesn’t work or you have questions, feel free to reach out!  

- Telegram: [@Loleg_Rus](https://t.me/Loleg_Rus)  
- Discord: [loleg5434](https://discord.com/users/loleg5434)

Thanks for using my mod! Enjoy! 🎉
